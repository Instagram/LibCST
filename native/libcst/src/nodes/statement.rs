...
impl<'r, 'a> Inflate<'a> for DeflatedClassDef<'r, 'a> {
    ...
    fn inflate(mut self, config: &Config<'a>) -> Result<Self::Inflated> {
        ...
        let (
            mut whitespace_after_name,
            mut type_parameters,
            mut whitespace_after_type_parameters,
            mut lpar,
            mut bases,
            mut keywords,
            mut rpar,
        ...
        if let Some(tparams) = self.type_parameters {
            ...
            lpar = self.lpar.inflate(config)?;
            bases = self.bases.inflate(config)?;
            keywords = self.keywords.inflate(config)?;
            rpar = self.rpar.inflate(config)?;
        }
